...
exports.handler = async function(event, context) {
  ...
  if (method === "GET") {
    if (event.path === "/subway/schedules") {
      params.sql = 'select v.*, s.stop_time, s.stop from vehicles v right join schedules s on v.id = s.vehicle_id where v.vehicle_type = "subway"';
